...
## 起動方法
- 前提: JDK 25、Gradle が利用可能であること。
- 実行:
  - Windows: `gradle.bat run`
  - macOS/Linux: `gradle run`
- 代替: Gradle Wrapperを利用する場合は `./gradlew run`（まだ同梱していないため、必要なら生成してください）。

## 操作説明（要点）
- タイトル: 開始/終了
- ステータス配分: 力/技/守備を合計18ptで配分（各0〜10）。残り0で開始可能。
- バトル: 斬/突/打のいずれかを選択。相性により命中/ダメージが補正。
- 5人抜き: 各勝利後にHP+20回復（上限100）。敗北でゲームオーバー。
